...
const fetchInitialData = async () => {
  ...
  try {
    // Run requests in parallel for assets and lookups
    const [assetRes, catRes, typesRes, locRes, deptRes, deprRes] =
      await Promise.all([
        ...
        DepreciationService.listDepreciations(),
      ]);
    ...
    if (Array.isArray(typesRes)) {
      assetTypes.value = typesRes;
    } else {
      assetTypes.value = typesRes.data?.data || typesRes.data || [];
    }
    ...
    if (Array.isArray(deprRes)) {
      depreciationMethods.value = deprRes;
    } else {
      depreciationMethods.value = deprRes.data?.data || deprRes.data || [];
    }
  } catch (error) {
...
const filteredAssets = computed(() => {
  ...
  if (selectedCategory.value) {
    filtered = filtered.filter(
      (asset) => asset.category_id == selectedCategory.value,
    );
  }
...
const stats = computed(() => ({
  ...
  totalValue: assets.value.reduce(
    ...
    >
      <div>
        ...
        <p class="text-slate-500 mt-1">
          Manage and track organizational assets and their lifecycles
        </p>
      </div>
      <div class="flex gap-3">
        <button
          class="px-5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm flex items-center gap-2"
        >
    ...
    <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
      ...
      >
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            class="flex items-center gap-2 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 hover:bg-slate-100 hover:border-slate-300 transition-all"
          >
